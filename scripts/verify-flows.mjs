...
async function runTests() {
  ...
  try {
    ...
    const { data: muni, error: muniErr } = await supabaseAdmin.from("municipalities").select("id").limit(1).single();
    if (muniErr || !muni) {
    ...
    const planMap = {};
    plans.forEach(p => { planMap[p.slug] = p.id; });
    ...
    const patientUser = await adminCreateAndSignIn(patientEmail, testPassword, "Test Patient", patientClient);
    ...
    const profUser = await adminCreateAndSignIn(professionalEmail, testPassword, "Test Professional", profClient);
    ...
    const { data: initialProfRoles, error: initialProfRolesErr } = await profClient.from("user_roles").select("role").eq("user_id", profUser.id);
    ...
    if (hasProfRole && hasNoPatientRole) {
      console.log("🟢 SUCCESS: User role automatically updated from 'patient' to 'professional' via database trigger!");
    } else {
      throw new Error(`❌ FAIL: Role synchronization failed. Roles are: ${JSON.stringify(updatedProfRoles)}`);
    }
    ...
    console.log(`🟢 SUCCESS: Professional owner can read/download their own onboarding verification documents!`);
    ...
    if (publicLookup === null) {
      console.log("🟢 SUCCESS: Draft/unpublished profiles are completely hidden from standard public visitors/patients!");
    } else {
    ...
    const centerUser = await adminCreateAndSignIn(centerEmail, testPassword, "Test Center", centerClient);
    ...
    if (hasCenterRole && hasNoPatientRoleCenter) {
      console.log("🟢 SUCCESS: User role automatically updated from 'patient' to 'center' via database trigger!");
    } else {
      throw new Error(`❌ FAIL: Role synchronization failed. Roles are: ${JSON.stringify(updatedCenterRoles)}`);
    }
    ...
    if (publicLookupCenter === null) {
      console.log("🟢 SUCCESS: Draft/unpublished center profiles are completely hidden from standard public visitors/patients!");
    } else {
    ...
    if (adminLoginErr) {
      throw new Error(`Admin login failed: ${adminLoginErr.message}. Ensure demo admin has been seeded.`);
    }
    ...
    console.log(`⚙️  Admin downloading professional's verification document from private bucket...`);
    ...
    if (downloadedText === testDocContent) {
      console.log(`🟢 SUCCESS: Admin can successfully review and download verification documents from the private bucket!`);
    } else {
    ...
    console.log(`✅ Professional profile published! Status: ${approvedProf.status}, Verified: ${approvedProf.verified}`);
    ...
    if (finalPublicLookup.status === "published" && finalPublicLookup.verified === true) {
      console.log("🟢 SUCCESS: Published professional profile is now fully visible to public visitors/patients!");
    } else {
